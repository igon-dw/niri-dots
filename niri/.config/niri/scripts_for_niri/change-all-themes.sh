...
# Apply to kitty if theme exists
...
# Apply to ghostty if theme exists
if [ -f "$GHOSTTY_THEMES_DIR/$SELECTED_THEME.conf" ]; then
	if ~/niri-dots/ghostty/.config/ghostty/scripts_for_ghostty/switch-theme.sh "$SELECTED_THEME"; then
		GHOSTTY_SUCCESS=true
	else
		GHOSTTY_SUCCESS=false
	fi
else
	notify-send "Theme '$SELECTED_THEME' not found in Ghostty" "Please select a Ghostty theme manually" -u normal
	if ~/niri-dots/ghostty/.config/ghostty/scripts_for_ghostty/switch-theme.sh; then
		GHOSTTY_SUCCESS=true
	else
		GHOSTTY_SUCCESS=false
	fi
